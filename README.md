...
完整配置示例见 [默认配置](./config/default.yaml)，质量更新说明见 [API 调用质量更新说明](./docs/api_call_p0_p1_quality_changelog.txt)。
...
- chat、chat_stream、embeddings 会尽量使用统一的错误归一化、自动重试和事件记录逻辑。
...
- [x] API 调用稳定性优化（错误归一化、HTML 错误清洗、可配置自动重试、URL 规范化）
- [x] API 调用能力增强（extra_headers、Provider capabilities、list_models、流式 usage/finish/status/error 元信息）
- [x] P0/P1 API 调用质量收尾（embeddings 重试、raw_arguments 保留、Responses 流式异常事件处理）
