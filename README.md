...
[**Changelog 模板**](./changelog/README.md)
...
GensokyoAI 对外发布版本采用日期版本号，例如 `v2026.5.11.0`；Python 包版本不带 `v`，Runtime 协议版本也使用日期版本号但兼容性优先看 `protocol_major_version`，持久化 schema version 继续使用整数。完整规则见 [版本管理说明](./docs/versioning.md)。

对外更新日志模板位于 [Changelog 模板](./changelog/README.md)，正式发布时建议在 `changelog/` 下按版本创建独立文件，例如 `changelog/v2026.5.11.0.md`。
